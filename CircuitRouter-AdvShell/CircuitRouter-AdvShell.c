...
int main (int argc, char** argv) {
    ...
    while (1) {
        ...
        if((select(pipeFD + 1, &fdSet, NULL, NULL, NULL)) < 0) {
          perror("select");
          exit(EXIT_FAILURE);
        }

        if (FD_ISSET(0, &fdSet)) {
          numArgs = readLineArguments(args, MAXARGS+1, buffer, BUFFER_SIZE, 0);
          ...
          numArgs = readLineArguments(args, MAXARGS+1, buffer, BUFFER_SIZE, pipeFD);
          ClientPipe = args[2];
        }
        ...
        if (numArgs > 0 && strcmp(args[0], COMMAND_RUN) == 0){
            ...
            if (pid > 0) {
                ...
                char *newArgs[3] = {seqsolver, args[1], ClientPipe};
                printf("UNIQUEID--->[%d];PID--->[%d]\n", id, getpid());
